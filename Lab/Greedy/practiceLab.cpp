...
void ower_own(vector<Process> processes, int flag)
{
    ...
    while (completedProcess < numProcesses)
    {
        ...
        else
        {
            ...
            while (!q.empty())
            {
                if (processes[q[0].second].startTime == -1)
                    processes[q[0].second].startTime = currentTime;
                if (!flag)
                    cout << currentTime << ' ';
                if (!flag)
                    cout << "P" << processes[q[0].second].processNumber << ' ';
                ll x = min(tq, q[0].first);
                processes[q[0].second].endTime = currentTime+x;
                processes[q[0].second].remainingBurstTime -= x;
                if (processes[q[0].second].remainingBurstTime == 0)
                    completedProcess++;
                q.erase(q.begin());
                for (int i = 0; i < numProcesses; i++)
                {
    ...
    if (!flag)
    {
        cout << endl; 
        sort(processes.begin(),processes.end(), compareProcessNo); 
        totalWait = 0, totalTurnaround = 0, avgresponse = 0; 
        for(int i = 0;i<(int)processes.size(); i++)
        {
            ...
            processes[i].waitingTime = processes[i].endTime-processes[i].arrivalTime-processes[i].burstTime; 
            processes[i].turnaroundTime = processes[i].endTime - processes[i].arrivalTime; 
            cout << "Waiting Time: " << processes[i].waitingTime << " ";
            totalWait += processes[i].waitingTime; 
            cout << "Turnaround Time: " << processes[i].turnaroundTime << endl;
...
void DABRR(vector<Process> &processes, int flag)
{
    int n = processes.size();
    vector<Process> ready_queue;
    vector<Process> completed_processes;
    ...
    while (!processes.empty() || !ready_queue.empty())
    {
        ...
        Process currentProcess = ready_queue.front();
        ...
        for (auto i : ready_queue)
            tq += i.remainingBurstTime;
        if (ready_queue.size())
            tq /= ready_queue.size();
        // tq = max(1LL, tq); 
        int executionTime = min(tq, (ll)currentProcess.remainingBurstTime);
        ...
        if (!flag)
        {
            cout << "P" << currentProcess.processNumber << " " << currentTime << " ";
        }
        ...
        if (currentProcess.remainingBurstTime == 0)
        {
            ...
            totalWatingTime += currentProcess.waitingTime;
            totalTurnaroundTime += currentProcess.turnaroundTime;
            currentProcess.endTime = currentTime;
            completed_processes.push_back(currentProcess);
        }
        else
        {
            while (!processes.empty() && processes.front().arrivalTime <= currentTime)
    ...
    double avg_waitingTime = (totalWatingTime * 1.0) / n;
    double avg_turnaroundTime = (totalTurnaroundTime * 1.0) / n;
    double avg_responseTime = (totalResponseTime * 1.0) / n;
